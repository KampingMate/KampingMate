...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
	...
	// 글쓴이 아이디로 검색
	@Query(value = "SELECT r.id , r.review_seq , r.kakao_id, r.kakao_name, r.title, r.content, r.write_date , r.cnt, r.goodpoint, r.bookmark r.reviewrate FROM Review r JOIN Member_Data m ON r.id = m.no_data WHERE m.id = ?1", nativeQuery = true)
	...
	public Page<Review> findReviewBykakao_nameContainingOrderBykakao_name(String kakao_name, int review_seq,
			Pageable pageable);
